from nlpboost import AutoTrainer, DatasetConfig, ModelConfig, joinpaths, ResultsPlotter
...
from nlpboost.hftransformers_manager import MultilabelTrainer
...
def test_autotrainer_train():
    ...
    wnli_config = {
        ...
        "hf_load_kwargs": {"path": "IIC/wnli_tests"},
        "label_col": "label",
        "is_2sents": True,
...
def test_autotrainer_qa_train():
    ...
    sqac = {
        ...
        "task": "qa",
        "text_field": "context",
        "hf_load_kwargs": {"path": "IIC/sqac_tests"},
        "label_col": "answers",
        "pre_func": _silly_pre_func,
        "smoke_test": False,
        "squad_v2": False,
...
def test_train_encoder_decoder():
    """Test training with an encoder-decoder model."""
    fixed_train_args = {
        "evaluation_strategy": "steps",
        "num_train_epochs": 1,
        "do_train": True,
    ...
    model_config = ModelConfig(
        ...
        random_init_trials=1,
        trainer_cls_summarization=Seq2SeqTrainer,
        model_cls_summarization=EncoderDecoderModel,
        encoder_name="dccuchile/bert-base-spanish-wwm-cased",
        decoder_name="dccuchile/bert-base-spanish-wwm-cased",
    )
    autotrainer = AutoTrainer(
        model_configs=[model_config],
        dataset_configs=[sqac],
...
def test_model_init():
    ...
    transformers_manager = HFTransformersManager(model_config, wnli_config)
    # train_args = autotrainer._load_train_args(wnli_config, model_config, ".")
    config = AutoConfig.from_pretrained("CenIA/albert-tiny-spanish")
...
def test_load_trainer():
    ...
    data_collator = transformers_manager.load_data_collator(tokenizer)
    dataset = load_dataset("IIC/wnli_tests")
